# 文件路径: MyDataWorkbench/tools/image_processor/pyside_tool.py
...
class Config:
    ...
    MIN_TOTAL_AREA_RATIO = 0.0001
    MIN_ASPECT_RATIO = 0.5
    MAX_ASPECT_RATIO = 5.0

# --- Backend Logic ---
# These functions are called by the worker threads.
...
class ImageProcessorWidget(QWidget):
    ...
    def start_download(self):
        ...
        self.download_button.setEnabled(False); self.download_progress.setValue(0)
    ...
    def start_filtering(self): self.filter_button.setEnabled(False); self.filter_progress.setValue(0); self.start_thread(FilterWorker, self.update_filter_progress, self.on_filter_finished)
    ...
    def start_processing(self): self.process_button.setEnabled(False); self.process_progress.setValue(0); self.start_thread(TemplateWorker, self.update_process_progress, self.on_process_finished, self.state['match_threshold'])
    ...
    def start_validation(self): self.validate_button.setEnabled(False); self.validation_results.setText("正在校验..."); self.start_thread(ValidationWorker, None, self.on_validation_finished)
    def on_validation_finished(self, missing_files): self.validate_button.setEnabled(True); self.validation_results.setText(f"✅ 所有文件均已处理。\n" if not missing_files else f"🟡 发现 {len(missing_files)} 个缺失文件:\n" + "\n".join(missing_files)); QMessageBox.information(self, "完成", "校验完成。")
    ...
    def select_qc_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "选择 qc.txt", "", "Text Files (*.txt)");
    ...
    def update_folder_status(self):
        unprocessed = sum(len(f) for _,_,f in os.walk(Config.UNPROCESSED_FOLDER)); processed = sum(len(f) for _,_,f in os.walk(Config.PROCESSED_FOLDER))
        self.unprocessed_label.setText(f"🔵 待处理: {unprocessed}"); self.processed_label.setText(f"🟢 已处理: {processed}")
    def refresh_template_list(self): self.template_list.clear(); self.template_list.addItems([f for f in os.listdir(Config.TEMPLATE_DIR) if f.lower().endswith(('.png', '.jpg'))])
    ...
    def delete_template(self):
        ...
        if selected: os.remove(os.path.join(Config.TEMPLATE_DIR, selected.text())); self.refresh_template_list()
    def reset_all(self):
        if QMessageBox.question(self, "确认重置", "确定要重置所有进度和文件吗？", QMessageBox.Yes|QMessageBox.No) == QMessageBox.Yes:
            if os.path.exists(Config.STATE_FILE_PATH): os.remove(Config.STATE_FILE_PATH)
            for folder in [Config.PROCESSED_FOLDER, Config.UNPROCESSED_FOLDER]: shutil.rmtree(folder, ignore_errors=True)
    ...
    def ensure_dirs_exist(self):
        for d in [Config.INPUT_DIR, Config.OUTPUT_DIR, Config.TEMPLATE_DIR, Config.PROCESSED_FOLDER, Config.UNPROCESSED_FOLDER]: os.makedirs(d, exist_ok=True)
    def load_state(self):
        if os.path.exists(Config.STATE_FILE_PATH):
            try: self.state = json.load(open(Config.STATE_FILE_PATH))
            except: self.state = {}
        defaults = {'current_step': 0, 'match_threshold': 0.8}; [self.state.setdefault(k,v) for k,v in defaults.items()]
    def save_state(self): json.dump(self.state, open(Config.STATE_FILE_PATH, 'w'), indent=4)
    def closeEvent(self, event):
        self.save_state()
        for thread in self.threads: thread.quit(); thread.wait()
