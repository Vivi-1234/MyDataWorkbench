...
class ImageProcessorWidget(QWidget):
    def __init__(self, main_window=None):
        ...
        sidebar_layout.setFixedWidth(200)
        sidebar_layout.addWidget(QLabel("<b>文件状态</b>"))
        self.unprocessed_label = QLabel("🔵 待处理: 0")
        self.processed_label = QLabel("🟢 已处理: 0")
        sidebar_layout.addWidget(self.unprocessed_label)
        sidebar_layout.addWidget(self.processed_label)
        sidebar_layout.addStretch()
        sidebar_layout.addWidget(QLabel("<b>重置操作</b>"))
        reset_button = QPushButton("🗑️ 全部重置")
        reset_button.clicked.connect(self.reset_all)
        sidebar_layout.addWidget(reset_button)

        content_layout = QVBoxLayout()
        title = QLabel("🖼️ 图片批量处理器")
        title.setObjectName("title")
        content_layout.addWidget(title)
        content_layout.addWidget(QLabel("本工具将引导您完成从下载到处理的全过程。"))
