...
def load_features(sort_by='created_at', order='desc'):
    ...
    try:
        ...
        # 使用 .get() 代替 .stream()，并保持10秒超时
        docs = db.collection('prototypes').order_by(sort_by, direction=direction).get(timeout=10)
        features = [{'id': doc.id, **doc.to_dict()} for doc in docs]
        return features
    except Exception as e:
